...
module.exports = class KubeClass {
  ...
  async getCoreApis() {
    ...
    if (coreApiList.statusCode !== 200) {
      return Promise.reject({ statusCode: coreApiList.statusCode, body: coreApiList.body });
    }
  ...
  async getApis(getAll = false) {
    ...
    if (apiQuery.statusCode !== 200) {
      return Promise.reject({ statusCode: apiQuery.statusCode, body: apiQuery.body });
    }
  ...
  async getApisSingle(groupVersion, krmHandler) {
    ...
    if (response.statusCode == 200 && objectPath.get(response, 'body.kind') === 'APIResourceList') {
      ...
      resources.map((r) => {
        let krm = new KubeResourceMeta(`/apis/${objectPath.get(response, 'body.groupVersion')}`, r, this._kubeApiConfig);
        krmHandler(krm);
      });
