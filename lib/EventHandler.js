...
module.exports = class EventHandler {
  constructor(params = {}) {
    ...
    if (!Number.isInteger(this._livenessEndHours)) {
      this._livenessEndHours = (Number.parseInt(this._livenessEndHours) >= 0) ? Number.parseInt(this._livenessEndHours) : 0;
    }
    ...
    if (!Number.isInteger(this._watchTimeoutSeconds)) {
      this._watchTimeoutSeconds = Number.parseInt(this._watchTimeoutSeconds) > 0 ? Number.parseInt(this._watchTimeoutSeconds) : 300;
    ...
    if (this._livenessInterval) {
      ...
      setInterval(() => {
        let now = Date.now();
        let millisecondsSinceLivenessStart = now - livenessStartTime;
        let hoursSinceLivenessStart = millisecondsSinceLivenessStart / 1000 / 60 / 60;
        let restartThresholdHours = this._livenessEndHours; // stop touching liveness file after the specified number of hours, which should tell kube to restart the pod
    ...
    setInterval(() => {
      let now = Date.now();
      let millisecondsSinceLastStart = now - this._wm.watchStart;
      let resetThresholdMilliseconds = (this._watchTimeoutMilliseconds + 10000); // interval time and kube timing is not exact, so adding 10 seconds to allow for variance without having to wait for an entire extra cycle
