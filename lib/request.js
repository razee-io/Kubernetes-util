...
const requestTriggerFile = 'USE_LEGACY_REQUEST_LIBRARY';
...
function getStream( requestOptions ) {
  const useLegacyRequest = fs.pathExistsSync(`./${requestTriggerFile}`);
  if( useLegacyRequest ) {
    return request( requestOptions );
  }

  // Use a Transform stream (both readable and writable)
  const axiosStream = new Stream.Transform({
  ...
  axiosStream.abort = abortController.abort;
...
async function doRequest( requestOptions ) {
  const useLegacyRequest = await fs.pathExists(`./${requestTriggerFile}`);
  if( useLegacyRequest ) {
    const requestResponse = await requestPromiseNative( requestOptions );
