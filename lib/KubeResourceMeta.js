...
module.exports = class KubeResourceMeta {
  ...
  async request(reqOpt) {
    ...
    return request(merge.all([this._kubeApiConfig, reqOpt, this._extraHeaders]));
  }

  async get(name, ns, reqOpt = {}) {
    const uri = this.uri({ name: name, namespace: ns });
    this._logger.debug(`Get ${uri}`);
    return request.get(merge.all([this._kubeApiConfig, reqOpt, { uri: uri, json: true }, this._extraHeaders]));
  }
  ...
  async put(file, reqOpt = {}) {
    ...
    return request.put(merge.all([this._kubeApiConfig, reqOpt, { uri: uri, json: file }, this._extraHeaders]));
  }

  async post(file, reqOpt = {}) {
    const uri = this.uri({ namespace: objectPath.get(file, 'metadata.namespace') });
    this._logger.debug(`Post ${uri}`);
    return request.post(merge.all([this._kubeApiConfig, reqOpt, { uri: uri, json: file }, this._extraHeaders]));
  }
  ...
  async patch(name, ns, jPatch, reqOpt = {}) {
    ...
    return request.patch(merge.all([this._kubeApiConfig, reqOpt, this._extraHeaders]));
  }
  ...
  async delete(name, ns, reqOpt = {}) {
    ...
    return request.delete(merge.all([this._kubeApiConfig, reqOpt, { uri: uri, json: true }, this._extraHeaders]));
  }
