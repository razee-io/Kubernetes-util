...
module.exports = class KubeResourceMeta {
  ...
  async request(reqOpt) {
    //PLC need to find anything that calls this and check/update its reqOpt and response expectations
    const axiosOptions = merge( reqOpt, { baseURL: reqOpt.baseUrl, url: reqOpt.uri, data: reqOpt.body }); // request: baseUrl/uri/body, axios: baseURL/url/data
  ...
  async mergePatch(name, ns, mPatch, reqOpt = {}) {
    ...
    return this.request(merge.all([reqOpt, { uri, data: mPatch }]));
  }
  ...
  async strategicMergePatch(name, ns, smPatch, reqOpt = {}) {
    ...
    return this.request(merge.all([reqOpt, { uri, data: smPatch }]));
  }
