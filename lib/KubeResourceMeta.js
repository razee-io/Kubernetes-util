...
module.exports = class KubeResourceMeta {
  ...
  async request(reqOpt) {
    ...
    reqOpt.data = reqOpt.body; delete reqOpt.body; // request: body, axios: data
    reqOpt.url = reqOpt.uri; delete reqOpt.uri; // request: uri, axios: url
    //PLC need to find anything that calls and check/update its reqOpt an response expectations
    return axios(merge.all([this.kubeApiConfig(), this._extraHeaders, reqOpt]));
  }

  async get(name, ns, reqOpt = {}) {
    const uri = this.uri({ name: name, namespace: ns });
    this._logger.debug(`Get ${uri}`);
    return this.request(merge.all([reqOpt, { url: uri, method: 'get' }]));
  }
  ...
  async patch(name, ns, jPatch, reqOpt = {}) {
    ...
    return this.request(merge.all([reqOpt, { url: uri, method: 'patch', data: jPatch }]));
  }
  ...
  async mergePatch(name, ns, mPatch, reqOpt = {}) {
    ...
    return this.request(merge.all([reqOpt, { url: uri, data: mPatch }]));
  }
  ...
  async strategicMergePatch(name, ns, smPatch, reqOpt = {}) {
    ...
    return this.request(merge.all([reqOpt, { url: uri, data: smPatch }]));
  }

  async delete(name, ns, reqOpt = {}) {
    const uri = this.uri({ name: name, namespace: ns });
    this._logger.debug(`Delete ${uri}`);
    return this.request(merge.all([reqOpt, { uri: uri, method: 'delete' }]));
  }
