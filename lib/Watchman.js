...
module.exports = class Watchman {
  constructor(options, objectHandler) {
    ...
    if (options.kubeResourceMeta === undefined || (typeof options.kubeResourceMeta.uri) !== 'function') {
      throw 'Watchman options.kubeResourceMeta.uri() must be a function.';
    }
    this._objectHandler = objectHandler;
    this._kubeResourceMeta = options.kubeResourceMeta;
  ...
  watch() {
    ...
    parser.on('data', (data) => {
      if (data.type === 'ERROR') {
        ...
        let metadata = { name: objectPath.get(data, 'object.metadata.name'), namespace: objectPath.get(data, 'object.metadata.namespace') };
        objectPath.set(data, 'object.metadata.annotations.selfLink', this._kubeResourceMeta.uri(metadata));
