...
module.exports = class Watchman {
  ...
  watch() {
    ...
    this._requestStream = RequestLib.getStream(merge(KubeApiConfig(), this._requestOptions))
    ...
    parser.on('data', (data) => {
      if (data.type === 'ERROR') {
        ...
        if( this._requestStream && this._requestStream.abort ) this._requestStream.abort(); //PLC FIX During automated test, the call to logger.error will call end() on the Watchman instance, destroying _requestStream
      } else {
