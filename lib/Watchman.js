...
module.exports = class Watchman {
  constructor(options, objectHandler) {
    ...
    The underlying library may not support all parameters however.  These are allowed:
    - uri
    - qs
    These parameters are supported by the `request` library, though it is not used here as it is deprecated.
    Instead these parameters are recognized and converted into the corresponding `axios` library parameters.
    This `Watchman` class is used by razee-io/WatchKeeper, which specifies both `uri` and `qs`.
    */
