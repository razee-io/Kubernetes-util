...
module.exports = class Watchman {
  constructor(options, objectHandler) {
    /*
    Note that options.requestOptions can be passed, providing `request` parameters.
    ...
    Instead these parameters are recognized and converted into the corresponding parameters for the library
    that is actually used (axios):
    - baseUrl -> baseURL
    - uri -> url
    - qs -> params
    E.g. This `Watchman` class is used by razee-io/WatchKeeper, which specifies both `uri` and `qs`.
    Using any request parameters outside `uri` and `qs` are not supported.
    */
    const allowedReqOptions = ['baseUrl','uri', 'qs'];
    ...
    if( invalidRequestOptions.length > 0 ) {
      throw `Watchman invalid request options: ${invalidRequestOptions.join(',')}.`;
    }
      ...
      [
        {
          ...
          baseUrl: kac.baseUrl, // needed for validUrl check, but no other KubeApiConfig values here so that code can fetch them before each call to watch().
        },
  ...
  async watch() {
    ...
    const axiosOptions = merge.all([KubeApiConfig(), this._requestOptions, { baseURL: this._requestOptions.baseUrl, url: this._requestOptions.uri, params: this._requestOptions.qs, responseType: 'stream' }]); // request: baseUrl/uri/qs, axios: baseURL/url/responseType/params
